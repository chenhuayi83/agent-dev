...
两级必须并存的原因是两种典型错配:**结果对但轨迹烂**(agent 直接改测试让它通过、绕过权限检查、或用 30 步和 10 倍成本蒙对——只测结果就是在**奖励作弊**/reward hacking,这是 agent 评估最贵的坑);**结果错但轨迹好**(外部依赖抖动、fixture 脏了——只看结果会把环境问题记到模型头上)。
...
- 误区:换了 judge 模型后直接对比新旧分数。换 judge = 换量具,必须在锚定集上重新标定并说明换算关系,否则"质量提升"可能只是量具变松了;同理,rubric 也不是越细越好——维度越多、档位越细,judge 自一致性掉得越快,宁可多个二元 judge,不要一个十档 judge。
...
1. **任务源**(题目哪来的、有没有可能进过训练语料)与 **环境**(能否可复现地重建:容器镜像、依赖锁定、外部依赖 mock);
2. **验证器**(怎么判成功,执行验证还是模型判定)与 **scaffold 契约**(agent 能用什么工具、最多多少步、什么模型档位)。**scaffold 不固定,分数就不可比**——这是跨表比较分数时最常见的错误。
...
- 排名随 scaffold 变化而变化——说明测的是 harness 不是模型。任一条满足,继续报这个数就是营销而非评估。
...
**这是当前最重要的行业信号,面试必须知道**(as-of 2026-08-19,**置信度:高**——官方页面与官方 X 帖均存在并互相印证,但 openai.com 原文抓取返回 403,具体数字引用前建议再核):**OpenAI 已公开宣布不再报告 SWE-bench Verified,并推荐行业改报 SWE-bench Pro**。三条理由层层递进:① **测试用例缺陷**——审计 o3 持续失败的 138 个问题,**59.4% 的测试用例有缺陷**(过窄的强制实现细节、过宽的检查题面外功能),即近六成"失败"不代表模型不行;② **训练污染**——2026-02-23 的分析发现主要前沿模型(GPT-5.2、Claude Opus 4.5、Gemini 3 Flash 等)均有在基准解答上训练过的证据;③ **官方结论**:Verified 上的进步已不再反映真实软件开发能力,而越来越多地反映训练时接触该基准的程度。**落差数据极具冲击力:Verified 上约 80% 的模型,在抗污染的 SWE-bench Pro 上仅约 23%。**

答这题的高分姿势:先给这条事实,再指出"头部挤在几分内"只是**表象**(饱和),真正的病因是**污染 + 测试缺陷导致的测量失效**——两者的处置方式完全不同:饱和只需换更难的题,失效则意味着历史分数整体不可信。另有二手报道称该推荐一度被回撤,未获证实,可如实说明。

这条的**教训是硬的**:基准的选择带有厂商利益,任何"新标准"公告都要先看谁在推、谁受益。收口成一句方法论:公开基准回答"这个模型大概在什么档",私有 eval 回答"它在**我的**任务上行不行";前者用于选型,后者用于发版。把公开基准分数当验收标准的团队,是在用别人的数据分布替代自己的。
...
- 边界:采样必然丢罕见失败,**安全类事件(护栏触发、权限拒绝、注入检测命中)必须 100% 记录,不进采样池**;另外 trace 只解释"发生了什么",要解释"为什么这么决策"还得留存 thinking 内容与当时的完整上下文——这又是 PII 与成本的权衡。
...
**阶段 0:把"成功"变成断言**(最难,也最省后续力气)——对每个任务类型逼问业务方一句话:"这次算成功的**可观测证据**是什么?",追问到能写成代码为止。输出物是一张表:任务分类法(3-7 类)× 每类的验证器类型(执行验证 / 状态断言 / 精确匹配 / rubric+judge)。写不出验证器的类型先降级为"人工抽检 + rubric",并明确标注**暂不进 CI 门禁**。这一步没做完就往下走,是最常见的失败模式:后面所有数字都会失去意义,因为没人说得清它们在测什么。
...
单条 case 的结构如下,其中 `fixture` 是 agent eval 与 LLM eval 的最大区别;规模节奏则是**50 条能跑起来,150-300 条能做门禁,500+ 才谈得上切片统计**。

```yaml
- id: refund-013
  input: "帮我把订单 A-1001 退款,用户说收到的是破损商品"
  fixture:                      # 必须能一键建/毁一个确定性世界
    db_seed: seeds/orders_v3.sql
    mocks: {payment_api: fixtures/payment_ok.json, clock: "2026-08-19T10:00:00Z"}
  verifier:   verifiers/refund.py::verify_outcome      # 结果级
  invariants: verifiers/refund.py::verify_trajectory   # 轨迹级
...
- **快集**(每个 PR):20-50 条 + 纯程序化验证器 + 单次运行,目标 < 10 分钟 / < $1,抓"明显坏了";
...
- **回归即 case**:线上事故 → 复现 → 进回归集 → 永久保留,只增不减;judge 的提示词或模型变更同理等于更换量具,必须在锚定集上重新标定(见 Q2)。
...
```
evals/
  datasets/  anchor/(不可变锚定集) regression/(事故回流,只增不减)
             seed/(手写核心路径)   sampled/(生产 trace 标注回流)
  verifiers/ refund.py  code_patch.py  judges/hallucination.md
  fixtures/  seeds/  mocks/
  runner.py  # 建环境 → 跑 agent → 收 trace → 判定 → 写 JSON 报告
  thresholds.yaml

# .github/workflows/evals.yml(骨架):on: [pull_request]
...
- 误区:把线上指标当 eval。线上指标滞后且混杂(流量、用户、季节都在变),做不了因果归因;两者是互补的两条腿。
- 边界:低频高风险任务(月结、清算、事故处置)样本天然稀缺——靠合成 + **影子运行**(agent 跑但不生效,与人工结果对照)积累,别指望等到自然样本。
- 边界:当 eval 账单接近生产账单时,该做的是分层采样与缓存,而不是砍掉重复运行——**砍掉重复运行等于砍掉方差信息,是最坏的省钱方式**。
