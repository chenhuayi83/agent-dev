...
> 版本:v1.1(2026-08-19)。全部变更史见 `meta/EVOLUTION.md`。
...
队列条目分三类轨道(v1.1 起,用户指令新增技能/面试题轨道),取题与预算规则相同,产出物不同:

| 类型 | 产出目录 | 模板 | 性质 |
|---|---|---|---|
| 前沿 | `deep-dives/YYYY/` | deep-dive.md | 专题深研报告,append-only |
| 技能 | `skills/<tool>/` | skill-topic.md | Claude Code / Codex 等 vibe coding 实战技能,活文档可迭代 |
| 面试题 | `interview/` | interview-q.md | 按主题成组(每组 4-6 题讲透),活文档可补题 |
...
5. 周元复盘(`run_count % 7 == 0`):抽查近 7 天简报是否同质化;kb 园艺(合并重复、修剪过长时间线);队列健康(<5 题从 kb"开放问题"补队);**轨道配比检查:最近 7 个深内容产出中,技能+面试题 ≥3、前沿 ≥2**(配比可在此调整);验收近一周协议改动是否兑现"预期检验",未兑现 → `git revert` 回滚该改动。
