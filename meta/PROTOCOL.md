...
> 版本:v1.2(2026-08-19)。全部变更史见 `meta/EVOLUTION.md`。
...
队列条目分类轨道,取题与预算规则相同,产出物不同:

| 类型 | 产出目录 | 模板 | 性质 | 占每日 P2 槽位 |
|---|---|---|---|---|
| 前沿 | `deep-dives/YYYY/` | deep-dive.md | 专题深研报告,append-only | 是 |
| 技能 | `skills/<tool>/` | skill-topic.md | Claude Code / Codex 等 vibe coding 实战技能,活文档可迭代 | 是 |
| 面试题 | `interview/` | interview-q.md | 按主题成组,每题讲透,活文档 | **否**(见下) |

**面试题轨道状态(v1.2 起)**:基础库 **01–12 十二组 74 题**已于 2026-08-19 一次性建成——01–10 为 agent 应用工程与前沿(基础/上下文工程/MCP协议/多智能体/记忆/评估/安全/生产化/框架选型/编码代理),11–12 为面试基本盘(RAG / 大模型底层工程)。**日常运行不再新增题组**,改为在 P4 做增量维护(见 P4 第 6 条)。仅两种情况重新占用 P2 槽位:用户点名要新题组;或出现全新的一级主题(新协议/新范式成为独立考点)。

1. 取题:`state.current_deep_dive` 非空(跨日大主题)→ 续做;否则取 `meta/queue.md` 待研区最高优先级(不分类型);用户点名则覆盖。
2. 打法:1-2 次 WebSearch 定位一手资料 → 6-8 次 WebFetch 精读(官方文档/spec/源码/作者博文 优先于二手转述)→ 关键断言尽量两处独立来源交叉验证。技能/面试题类若主要依靠稳定知识,联网预算可下调,但**版本敏感断言必须当次核实并标 as-of**。
3. 前沿类按 `meta/templates/deep-dive.md` 产出,≤400 行:TL;DR ≤10 行;核心发现每个关键断言带来源+日期;实践启示;知识库回写清单;开放问题。技能/面试题类按各自模板产出,并满足**讲透标准**:

**讲透标准**(技能/面试题类产出的硬要求,五件套缺一不可):
- ① **本质**:是什么、为什么存在(解决什么问题)
- ② **机制**:内部怎么工作,从第一性原理推导(不是罗列口诀)
- ③ **实例**:可操作的命令/代码/对话示例
- ④ **边界**:何时不适用、常见误区/反模式
- ⑤ **检验**:读者如何自测掌握(练习/追问);面试题类必须含"追问预判"
...
3. kb 文件规范:front-matter(`last_updated` / `maturity: stub|growing|mature` / `deep_dives` 关联);"原理"(稳定)与"现状"(易变,断言带 as-of 日期)分节;单文件 ≤500 行,超限拆分或精炼(精炼=压缩表达,不减信息量)。`skills/` 与 `interview/` 文档同属沉淀层,适用同样的可编辑/front-matter/500 行规则,并与 kb 主题互相链接。
4. 更新 `kb/_index.md` 对应行(简介+成熟度)。
5. checkpoint 提交:`kb: <涉及文件>`
6. **面试题增量维护**(v1.2 起的每日固定动作,替代"每天出新题组"):检查当日简报与深研内容,按以下三档处理,**无可动则明确记录"面试题库无需更新"**——
   - **改变答案**(新事实使既有答案过时/不完整):修订对应题的"深入版"或"版本敏感"表述,标注 as-of 日期;
   - **值得成为新考点**(新协议/新范式/标志性事件):在最贴切的既有题组内**追加 1 题**(不新建组),并更新该组 front-matter 的 `questions` 数与 `interview/_index.md`;
   - **仅为素材**(可作为既有题的实例/追问弹药):补进对应题的"实例"或"追问预判"。
   提交:`interview: <组名> 增量(<一句话原因>)`
...
5. 周元复盘(`run_count % 7 == 0`):抽查近 7 天简报是否同质化;kb 园艺(合并重复、修剪过长时间线);队列健康(<5 题从 kb"开放问题"补队);**轨道配比检查:最近 7 个 P2 深内容中,技能 ≥3、前沿 ≥2**(配比可在此调整);**面试题库体检**:轮流抽查 1-2 组,核对其中的版本敏感断言(基准分数、spec 版本、产品能力)是否已过期,过期即修订并标注新的 as-of;验收近一周协议改动是否兑现"预期检验",未兑现 → `git revert` 回滚该改动。
