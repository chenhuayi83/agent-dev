...
- [规划与推理](00-fundamentals/planning-reasoning.md) — agent 如何决定做什么(stub)

## 01-context-engineering 上下文工程

- [上下文工程总览](01-context-engineering/overview.md) — 上下文是 agent 最稀缺的资源(stub)
...
- [记忆架构](05-memory/memory-architectures.md) — 短期(会话内)/长期(跨会话)记忆分层;文件式、向量式、图式(时序知识图谱)记忆的机制与取舍;写入策略(何时记、记什么)(stub)
...
- [基准测试](06-evals/benchmarks.md) — SWE-bench(Verified/Pro)、GAIA、tau-bench、terminal-bench、OSWorld 等 agent 基准的构成、饱和度与污染问题(stub)
...
- [成本与延迟优化](08-production/cost-latency.md) — prompt caching、模型分级(路由)、batch API、并行化、token 预算护栏;成本可观测(stub)
